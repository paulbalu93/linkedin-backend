...
router.get('/', requireLogin, async (req, res) => {
	try {
		...
		const post = await PostModel.find().populate('user', 'username').sort('-createdAt');
		res.send(post);
	} catch (error) {
		console.log(error);
	}
...
router.get('/:postId', requireLogin, async (req, res) => {
	try {
		const response = req.body;
		console.log(response);
		const post = await PostModel.findById(req.params.postId).populate('user', 'username').sort('-createdAt');
		res.send(post);
	} catch (error) {
		console.log(error);
	}
...
router.put('/:id', requireLogin, async (req, res, next) => {
	try {
		const modifiedPost = await PostModel.findByIdAndUpdate(req.params.id, req.body, {
			runValidators: true,
			new: true,
		});
		if (modifiedPost) {
